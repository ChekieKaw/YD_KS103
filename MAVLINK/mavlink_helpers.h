...
#endif
	mavlink_ck_a(msg) = (uint8_t)(msg->checksum & 0xFF);//mavlink_ck_a(msg) *((msg)->len + (uint8_t *)_MAV_PAYLOAD_NON_CONST(msg))
	mavlink_ck_b(msg) = (uint8_t)(msg->checksum >> 8);//*(((msg)->len+(uint16_t)1) + (uint8_t *)_MAV_PAYLOAD_NON_CONST(msg))
...
MAVLINK_HELPER uint16_t mavlink_finalize_message(mavlink_message_t* msg, uint8_t system_id, uint8_t component_id, 
						 uint8_t length)
{
	return mavlink_finalize_message_chan(msg,1 system_id, component_id, MAVLINK_COMM_0, length);
}
#endif
